...
namespace Chr.Avro.Confluent.Tests
{
    public class SchemaRegistryDeserializerBuilderTests
    {
        ...
        [Fact]
        public void BuildsLazyDeserializer()
        {
            using (var builder = new SchemaRegistryDeserializerBuilder(RegistryMock.Object))
            {
                var bytes = new byte[] { 0x00, 0x00, 0x00, 0x00, 0x0c, 0x00 };
                var deserialize = builder.BuildDeserializer<string>();

                RegistryMock.VerifyNoOtherCalls();

                Assert.Empty(deserialize(TestTopic, bytes, false));
        ...
        public async Task DeserializesConfluentWireFormat(byte[] data, string encoding)
        {
            using (var builder = new SchemaRegistryDeserializerBuilder(RegistryMock.Object))
            {
                ...
                Assert.Equal(encoding, deserialize(TestTopic, data, false));
            }
        }
        ...
        public async Task ThrowsOnSchemaIdMismatch(byte[] data)
        {
            using (var builder = new SchemaRegistryDeserializerBuilder(RegistryMock.Object))
            {
                ...
                Assert.Throws<InvalidDataException>(() => deserialize(TestTopic, data, false));
            }
        }
