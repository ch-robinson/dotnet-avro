namespace Chr.Avro.Tests
{
    ...
    public class TypeResolverWithReflectionCasesShould : TypeResolverShould
    {
        protected override TypeResolver Resolver => new (TypeResolver.CreateReflectionCaseBuilders());
        ...
        {
            var resolver = new TypeResolver(TypeResolver.CreateReflectionCaseBuilders(resolveUnderlyingEnumTypes: true));
        ...
        {
            var resolver = new TypeResolver(TypeResolver.CreateReflectionCaseBuilders(), resolveReferenceTypesAsNullable: true);
                ...
                c =>
                {
                    ...
                    Assert.Collection(
                        ...
                        p =>
                        {
                            ...
                            Assert.Equal(typeof(int), p.Parameter.ParameterType);
                        },
                        p =>
                        {
                            Assert.Equal("fieldB", p.Name.Value);
                            Assert.True(p.Parameter.IsOptional);
                            Assert.Equal(typeof(string), p.Parameter.ParameterType);
                        });
                });
        }
    }
