...
namespace Chr.Avro.Serialization
{
    ...
    /// <summary>
    /// Implements a <see cref="BinarySerializerBuilder" /> case that matches <see cref="TimeLogicalType" />
    /// and attempts to map it to <see cref="TimeOnly" />.
    /// </summary>
    public class BinaryTimeSerializerBuilderCase : TimeSerializerBuilderCase, IBinarySerializerBuilderCase
    {
        /// <summary>
        /// Builds a <see cref="BinarySerializer{T}" /> for a <see cref="TimeLogicalType" />.
        /// </summary>
        /// <returns>
        /// A successful <see cref="BinarySerializerBuilderCaseResult" /> if <paramref name="schema" />
        /// has a <see cref="TimeLogicalType" />; an unsuccessful <see cref="BinarySerializerBuilderCaseResult" />
        /// otherwise.
        /// </returns>
        ...
        public virtual BinarySerializerBuilderCaseResult BuildExpression(Expression value, Type type, Schema schema, BinarySerializerBuilderContext context)
        {
            if (schema.LogicalType is TimeLogicalType)
            {
                ...
                // return writer.WriteInteger((value.Ticks - midnight.Ticks) / factor);
            ...
            {
                return BinarySerializerBuilderCaseResult.FromException(new UnsupportedSchemaException(schema, $"{nameof(BinaryTimeSerializerBuilderCase)} can only be applied schemas with a {nameof(TimeLogicalType)}."));
            }
        }
    }
}
